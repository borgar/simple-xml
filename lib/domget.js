...
export const FILTERS = {
  ...
  'contains': (s, elms, not) => elms.filter(d => (d.textContent || '').includes(s) ^ not),

  'checked': (s, elms, not) => elms.filter(d => d.hasAttribute('checked') ^ not),

  'disabled': (s, elms, not) => elms.filter(d => d.hasAttribute('disabled') ^ not),

  'empty': (s, elms, not) => elms.filter(d => (!d.childNodes.length) ^ not),

  'onlychild': (s, elms, not) => elms.filter(d => (childrenOf(d.parentNode).length === 1) ^ not),

  'lastchild': (s, elms, not) => (
    elms.filter(d => {
